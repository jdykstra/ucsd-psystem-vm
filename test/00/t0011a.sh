...
program example;
  ...
  procedure gt(a, b: set40; expect: boolean);
  begin
    if ((a > b) = expect)
    then
      print('pass')
...
begin
  gt([1, 4, 7, 31, 37, 52], [1, 4, 7, 31, 37], true);
  gt([1, 4, 7, 31, 37    ], [1, 4, 7, 31, 37], false);
  gt([1, 4, 7, 31        ], [1, 4, 7, 31, 37], false);
end.
fubar
test $? -eq 0 || no_result
